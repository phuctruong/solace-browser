---
title: Manager Execution Specialist Approval Truth (SAC83)
type: prime-mermaid
---
...
### Context
In SAC83, the Dev Manager inspectability loop verifies honest next-step approval truth. `A2 (Target Assignment Approval Truth)` is only allowed to claim exact approval truth when approval state is read for the launched target assignment and the current request, source assignment, target assignment, role, and run stay aligned in the exact workflow-bound branch. If approval state is visible but the current binding has fallen back, Hub must say `fallback approval branch tracked` instead of overstating workflow continuity.
