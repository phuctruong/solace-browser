---
title: Manager Execution Inbox Packet Workflow (SAC77)
type: prime-mermaid
---
...
graph TD
    subgraph `solace-hub` (Dev Workspace)
        ...
        L1 -- "__solaceLastWorkflowLaunchAction" --> I
    end
...
### Context
In SAC77, the workflow loop constraint reaches stronger inspectability. The Manager no longer has to leave the workflow result box to see whether the launched next-step run produced a concrete `payload.json` packet. Because the `payload_exists` backend field is mapped into the launched assignment branch, the human governor can inspect the next-step worker packet against the exact routed assignment and launched run rather than a generic role summary.
