---
title: Manager Execution Specialist Approval Action Truth (SAC84)
type: prime-mermaid
---
...
### Context
In SAC84, the Dev Manager inspectability loop verifies honest next-step approval action truth. `A3 (Target Assignment Approval Action)` is only allowed to claim exact action truth when the manager action is bound to the launched target assignment and the current request, source assignment, target assignment, role, and run stay aligned in the exact workflow-bound branch. If the visible target is only reachable through fallback binding, Hub must say so instead of overstating exact downstream control.
