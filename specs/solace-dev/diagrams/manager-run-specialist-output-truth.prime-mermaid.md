---
title: Manager Execution Specialist Output Truth (SAC82)
type: prime-mermaid
---
...
### Context
In SAC82, the Dev Manager inspectability loop verifies honest specialist output truth. `T (Specialist Output Truth)` is only allowed to claim exact output when `report.html` exists for the launched next-step run and the current request, source assignment, target assignment, role, and run stay aligned in the exact workflow-bound branch. If report output exists but the current binding has fallen back, Hub must say `fallback output tracked` instead of overstating workflow continuity.
