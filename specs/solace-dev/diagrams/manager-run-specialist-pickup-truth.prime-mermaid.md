---
title: Manager Execution Specialist Pickup Truth (SAC80)
type: prime-mermaid
---
...
graph TD
    ...
    subgraph `solace-runtime` (Back Office / System)
        ...
        B6[apps/runs/ events runtime stream]
        ...
        B4 -. "payload fetch string" .-> P1
        B4 -. "report content string" .-> P2
        
        U -. "events.jsonl boolean status" .-> B6
    ...
    style B6 fill:#4c1d95,stroke:#a78bfa,color:#fff
```

### Context
In SAC80, the Dev Manager inspectability loop verifies honest specialist pickup truth. `U (Specialist Pickup Receipt Truth)` is only allowed to claim exact pickup when events exist for the launched next-step run and the current request, source assignment, target assignment, role, and run stay aligned in the exact workflow-bound branch. If events exist but the current binding has fallen back, Hub must say `fallback pickup tracked` instead of overstating continuity.
