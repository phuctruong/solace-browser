---
title: Manager Execution Specialist Destination Pickup Truth (SAC88)
type: prime-mermaid
---
...
### Context
In SAC88, the Dev Manager can verify whether the launched destination branch was actually picked up. `U2` is only exact when the nested run still matches the same request, the same source assignment and source run that produced the destination launch, the same destination assignment, and the same nested launched role and run. If events do not yet exist for that exact nested run, the workflow box must say it is still awaiting destination pickup truth instead of overstating completion.
