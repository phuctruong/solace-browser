...
if [[ -f "$PROJECT_ROOT/data/default/recipes/linkedin-update.recipe.json" ]]; then
    if grep -q '"locked": true' "$PROJECT_ROOT/data/default/recipes/linkedin-update.recipe.json"; then
...
if [[ $failed -eq 0 ]]; then
    ...
    echo "  4. Run: bash src/scripts/build-wish-21.0.sh"
    echo ""
    exit 1
fi
