...
Recompute to verify: `sha256sum data/default/swarms/README.mmd`
...
flowchart TD
  ...
  subgraph SKILLS["Skill Files (data/default/swarms/skills/)"]
...
1. Create `data/default/swarms/{agent-type}.md` following the template structure (sections 0-8)
...
4. Recompute `sha256sum data/default/swarms/README.mmd` and update `README.sha256`
5. Bump version in this document (MINOR bump for new agents)
6. Update `data/default/skills/README.md` to reference the new agent if it uses a new skill

## How To Update a Skill Pack

1. Edit the target `data/default/swarms/{agent-type}.md` skill_pack frontmatter and section 1
