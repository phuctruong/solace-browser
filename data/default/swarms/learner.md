...
Output targets: data/default/recipes/ | data/default/skills/learned-<slug>.md | AGENTS.md | scratch/.
...
files, write new skill draft files in `data/default/skills/` and `data/default/recipes/`, append to
...
1. `data/default/skills/prime-safety.md` — god-skill; wins all conflicts
2. `data/default/skills/prime-coder.md` — evidence contract, localization budget, witness lines
3. `data/default/skills/phuc-forecast.md` — FORECAST phase for failure-mode classification, premortem
...
Destination: `data/default/recipes/recipe.<slug>.md`
...
`data/default/skills/learned-<slug>.md` if no existing skill is a close match.
...
`data/default/skills/` or `data/default/swarms/`. Learner checks before writing; duplicate → discard.
...
classify(candidate):
  ...
  4. Does it substantially duplicate data/default/skills/ or data/default/swarms/ content?
     YES → DUPLICATE
...
| QG-4 | **Not duplicate** | Does not substantially duplicate any existing file in `data/default/skills/` or `data/default/swarms/` |
...
Written to `data/default/recipes/recipe.<slug>.md`. Must include frontmatter:
...
Written to `data/default/skills/learned-<slug>.md`. Must include frontmatter:
...
EXISTING_SKILLS: [list of files in data/default/skills/]
EXISTING_SWARMS: [list of files in data/default/swarms/]
EXISTING_RECIPES: [list of files in data/default/recipes/]
...
- **DUPLICATE_EXISTING_SKILL**: writing content that substantially duplicates
  existing files in `data/default/skills/` or `data/default/swarms/`. The Learner must read the existing
...
1. Learner emits `data/default/skills/learned-<slug>.md` at Lane C / Rung 641.
