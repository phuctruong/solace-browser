...
FORBIDDEN_PATTERNS:
  JSON_AS_SOURCE_OF_TRUTH:
    ...
    detect: "data/default/primewiki/*.json (outside archive/)"
  ...
  YAML_AS_SOURCE_OF_TRUTH:
    ...
    detect: "data/default/primewiki/*.yaml"
    fix: "Convert to PM triplet"

  ORPHANED_MMD:
    description: ".mmd file exists without matching .sha256 and .prime-mermaid.md"
    detect: "find data/default/primewiki/ -name '*.mmd' | check for matching triplet"
  ...
  STALE_JSON_IN_PRIMEWIKI:
    description: "JSON file exists in data/default/primewiki/ (not archive/)"
    detect: "ls data/default/primewiki/*.json"
    fix: "Move to archive/"
```
...
find data/default/primewiki/ -name "*.json" -not -path "*/archive/*"

# Check for orphaned .mmd files (no matching .sha256)
for mmd in data/default/primewiki/**/*.mmd; do
  sha="${mmd%.mmd}.sha256"
...
mkdir data/default/primewiki/{platform}

# 2. Write canonical Mermaid body
# data/default/primewiki/{platform}/{platform}-page-flow.mmd
# (State machine or flowchart, SCREAMING_SNAKE_CASE states)

# 3. Compute SHA256
sha256sum data/default/primewiki/{platform}/{platform}-page-flow.mmd > data/default/primewiki/{platform}/{platform}-page-flow.sha256

# 4. Write human spec
# data/default/primewiki/{platform}/{platform}-page-flow.prime-mermaid.md
# (Include: selector map, embedded diagram, drift detection command)

# 5. Add to PRIMEWIKI_INDEX.md

# 6. Commit the triplet
git add data/default/primewiki/{platform}/
