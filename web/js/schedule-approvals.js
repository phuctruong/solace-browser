...
(function () {
  ...
  function checkPendingQueue() {
    const pending = state.activities.filter(function (a) { return a.status === 'pending_approval'; });
    ...
    if (pending.length > 0) {
      ...
      const badge2 = document.getElementById('yyPendingBadge');
  ...
  function setupSignOffSheet() {
    const openBtn = document.getElementById('signoffOpenBtn');
    if (openBtn) openBtn.addEventListener('click', openSignOffSheet);
    const closeBtn = document.getElementById('signoffClose');
    if (closeBtn) closeBtn.addEventListener('click', closeSignOffSheet);
    const overlay = document.getElementById('signoffOverlay');
  ...
  function openSignOffSheet() {
    ...
    if (!pending.length) {
      ...
      pending.forEach(function (a) {
        ...
        const scopeStr = (a.scopes_used || []).map(function (s) {
  ...
  function renderApprovals() {
    ...
    const filtered = fn.getFiltered();
    const pending = filtered.filter(function (a) { return a.status === 'pending_approval' || a.status === 'cooldown'; });
    const approved = filtered.filter(function (a) { return a.status === 'success' && a.approved_by; }).slice(0, constants.MAX_APPROVED_SHOWN);
  ...
  function makeApprovalCard(a) {
    ...
    const time = utils.formatTime(a.started_at);
    const cost = a.cost_usd ? '$' + a.cost_usd.toFixed(4) : '\u2014';
    const scopeStr = (a.scopes_used || []).join(', ') || '\u2014';

    const safeId = utils.escapeHtml(a.id);
    const safeName = utils.escapeHtml(a.app_name || a.app_id);
    const safeStatus = utils.escapeHtml(a.status.replace(/_/g,' '));
    const safeTier = utils.escapeHtml(a.safety_tier || 'A');
    const safeHash = utils.escapeHtml(a.evidence_hash || '');
    ...
    const appNameEl = card.querySelector('.kanban-card__app');
    if (appNameEl) {
  ...
  function approveRun(id, withEsign) {
    ...
    const sessionId = sessionStorage.getItem('sb_session_id') || crypto.randomUUID();
    const body = {
      approved_by: approverName,
      device_id: localStorage.getItem('sb_device_id') || (function () { const id = crypto.randomUUID(); localStorage.setItem('sb_device_id', id); return id; })(),
    ...
    .then(function (data) {
      ...
      if (withEsign && data.esign_hash) {
        const act = state.activities.find(function (x) { return x.id === id; });
  ...
  function startCountdown(a, el) {
    ...
    const tick = function () {
      ...
      if (remaining <= 0) {
        ...
        const current = state.activities.find(function (x) { return x.id === a.id; });
