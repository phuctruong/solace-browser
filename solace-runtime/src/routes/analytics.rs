...
async fn summary(State(state): State<AppState>) -> Json<Value> {
        ...
        if let Some(dir) = crate::utils::find_app_dir(&app.id) {
            ...
            if runs_dir.exists() {
                total_runs += std::fs::read_dir(&runs_dir)
                    .into_iter().flatten().filter_map(|e| e.ok()).count();
            }
        }
    }
    ...
    for app_id in &bo_apps {
        let db_path = solace_home.join("backoffice").join(app_id).join("workspace.db");
        if db_path.exists() {
            if let Ok(conn) = rusqlite::Connection::open(&db_path) {
                // Count records in all tables
                if let Ok(mut stmt) = conn.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE '_%'") {
    ...
    let domain_count = apps.iter().map(|a| &a.domain).collect::<std::collections::HashSet<_>>().len();
...
async fn run_query(
    ...
    if !sql_upper.starts_with("SELECT") {
        return Err((StatusCode::FORBIDDEN, Json(json!({"error": "only SELECT queries allowed"}))));
    }

    let solace_home = crate::utils::solace_home();
    let db_path = solace_home.join("backoffice").join(&req.app_id).join("workspace.db");
    if !db_path.exists() {
        return Err((StatusCode::NOT_FOUND, Json(json!({"error": format!("database not found for {}", req.app_id)}))));
    }
    ...
    let mut stmt = conn.prepare(&req.sql)
        .map_err(|e| (StatusCode::BAD_REQUEST, Json(json!({"error": format!("sql: {e}")}))))?;

    let col_names: Vec<String> = stmt.column_names().iter().map(|s| s.to_string()).collect();
    let mut rows = stmt.query([])
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": format!("query: {e}")}))))?;

    let mut results = Vec::new();
    while let Some(row) = rows.next().map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": format!("row: {e}")}))))? {
...
async fn pipeline_funnel(
    ...
    let app_id = params.get("app_id").cloned().unwrap_or_else(|| "backoffice-crm".to_string());
    let table = params.get("table").cloned().unwrap_or_else(|| "contacts".to_string());
    let group_by = params.get("group_by").cloned().unwrap_or_else(|| "stage".to_string());
    ...
    if db_path.exists() {
        if let Ok(conn) = rusqlite::Connection::open_with_flags(&db_path, rusqlite::OpenFlags::SQLITE_OPEN_READ_ONLY) {
            let sql = format!("SELECT \"{}\", COUNT(*) as count FROM \"{}\" GROUP BY \"{}\" ORDER BY count DESC", group_by, table, group_by);
